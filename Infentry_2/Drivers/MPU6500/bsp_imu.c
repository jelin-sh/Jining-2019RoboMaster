...
uint8_t mpu_write_byte(uint8_t const reg, uint8_t const data)
{
	MPU_NSS_LOW;
	tx = reg & 0x7F;
	HAL_SPI_TransmitReceive(&MPU_HSPI, &tx, &rx, 1, 200);
	tx = data;
	HAL_SPI_TransmitReceive(&MPU_HSPI, &tx, &rx, 1, 200);
	MPU_NSS_HIGH;
	return 0;
...
uint8_t mpu_read_bytes(uint8_t const regAddr, uint8_t* pData, uint8_t len)
{
	...
	tx_buff[0] = tx;
	HAL_SPI_TransmitReceive(&MPU_HSPI, &tx, &rx, 1, 200);
	HAL_SPI_TransmitReceive(&MPU_HSPI, tx_buff, pData, len, 200);
	MPU_NSS_HIGH;
	return 0;
}
...
void mpu_offset_call(void)
{
	...
	mpu_data.ax_offset= (int16_t) (mpu_data.ax_offset / 300);
	mpu_data.ay_offset= (int16_t) (mpu_data.ay_offset / 300);
	mpu_data.az_offset= (int16_t) (mpu_data.az_offset / 300);
	mpu_data.gx_offset= (int16_t) (mpu_data.gx_offset / 300);
	mpu_data.gy_offset= (int16_t) (mpu_data.gx_offset / 300);
	mpu_data.gz_offset= (int16_t) (mpu_data.gz_offset / 300);
}
