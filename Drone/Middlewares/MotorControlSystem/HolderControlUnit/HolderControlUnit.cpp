...
void HolderControlUnit::Init() {
  ...
  motorYaw =
      new GM6020(CAN_ID_HODERYAW,
                 new PID(20000, 20000, 0, 0, 2000, 110, 500) /*速度环PID*/,
                 new PID(20, 0, 0, 0, 0.08, 0, 0) /*位置环PID*/);

  motorPitch =
      new GM6020(CAN_ID_HODERPITCH, new PID(20000, 20000, 0, 0, 3500, 170, 360),
                 new PID(50, 10, 0, 0, 0.08, 0, 0.01));
...
void HolderControlUnit::RunYaw(float angle) {
  ...
  targetCurrent = motorYaw->SpeedPIDOut(targetSpeed, currentSpeed);
...
void HolderControlUnit::RunPitch(float angle) {
  ...
  float currentSpeed = 0;
  float currentAngle = motorPitch->GetAngel();
  if (!AttitudeListener::IsBusy()) {
  ...
  if (currentAngle <= 0) {
    currentAngle += 360;
    motorPitch->SetAngle(currentAngle);
  }
...
void HolderControlUnit::UpdateControlParmForRemote() {
  ...
  if (RemoteListener::GetData().rc.ch2 > REMOTE_MID_VAL) {
    targetYawAngle += 0.6;
  } else if (RemoteListener::GetData().rc.ch2 < REMOTE_MID_VAL) {
    targetYawAngle -= 0.6;
  }
  if (RemoteListener::GetData().rc.ch1 > REMOTE_MID_VAL) {
    targetPitchAngle += 0.2;
  } else if (RemoteListener::GetData().rc.ch1 < REMOTE_MID_VAL) {
    targetPitchAngle -= 0.2;
  }
  ...
  //System::pc::println("target:%d",(int)targetYawAngle);
